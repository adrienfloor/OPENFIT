...
- **Daily wellness scores** (`packages/fitness-core/src/scores.ts`): transparent published composites, not Zepp reverse-engineering.
  ...
  - **`effortScore`** — PAI-style. Integrates %HRR (Karvonen) over 24 h HR samples with 5 intensity tiers (<40/40–60/60–80/80–90/≥90 % → 0/1/2/3/4 pts·min). Daily target 100 intensity-minutes → 100. Max HR via Tanaka (`calculateMaxHR`). Device-off gaps >10 min are dropped so idle HR-device windows don't fake activity.
